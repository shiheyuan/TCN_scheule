...
final class Item {
	...
	Item(final int size, final int value) {
		this.size = size;
		this.value = value;
	}
...
final class FF implements Function<Genotype<BitGene>, Double> {
	...
	public FF(final Item[] items, final int size) {
		this.items = items;
		this.size = size;
	}
	...
	public Double apply(final Genotype<BitGene> gt) {
		final Item sum = ((BitChromosome) gt.getChromosome()).ones().mapToObj(i -> items[i]).collect(Item.toSum());

		return (double) (sum.size <= this.size ? sum.value : 0);
	}
}
...
public class Knapsack {
	...
	public static void main(final String[] args) {
		...
		final Engine<BitGene, Double> engine = Engine.builder(ff, BitChromosome.of(nitems, 0.5)).populationSize(500)
				...
				// TournamentSelector<>(5)).offspringSelector(new
				// RouletteWheelSelector<>())
				// .alterers(new Mutator<>(0.115), new
